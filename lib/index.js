...
var SIZE_MIN = 64;
var SIZE_MAX = 2400;
function clampImageSize(value, label) {
...
// src/config.ts
var InlineImagesSettingsSchema = z.object({
  maxWidth: z.number().min(SIZE_MIN).max(SIZE_MAX).default(DEFAULT_MAX_WIDTH).description("\u5BF9\u8BDD\u6B63\u6587\u5185\u8054\u56FE\u7247\u7684\u6700\u5927\u5BBD\u5EA6, \u5355\u4F4D\u50CF\u7D20."),
  maxHeight: z.number().min(SIZE_MIN).max(SIZE_MAX).default(DEFAULT_MAX_HEIGHT).description("\u5BF9\u8BDD\u6B63\u6587\u5185\u8054\u56FE\u7247\u7684\u6700\u5927\u9AD8\u5EA6, \u5355\u4F4D\u50CF\u7D20.")
});
...
var InlineImagesRuntime = class extends TypertRemoteService {
  ...
  async setConfig(args) {
    ...
    if (owner === void 0) throw new Error("\u8BBE\u7F6E\u670D\u52A1\u5C1A\u672A\u5C31\u7EEA, \u8BF7\u7A0D\u540E\u91CD\u8BD5");
    const patch = {};
    ...
    if (patch.maxWidth !== void 0 || patch.maxHeight !== void 0) {
...
function apply(ctx) {
  ...
  runtime.ensureToken(
    (ref) => {
      const credentials = credentialsOf();
      return credentials === void 0 ? Promise.resolve(void 0) : credentials.resolve(ref);
    },
    (ref, value) => {
      const credentials = credentialsOf();
      return credentials === void 0 ? Promise.resolve() : credentials.set(ref, value);
    }
  ).then((fresh) => {
    if (token === void 0) {
      token = fresh;
